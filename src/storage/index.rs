...
impl LibraryIndex {
    ...
    pub async fn get_all_tracks(&self) -> Vec<Arc<TrackMetadata>> {
        self.tracks.read().await.values().map(|t| Arc::new(t.clone())).collect()
    }
    ...
    pub async fn get_playlist_tracks(&self, playlist_name: &str, music_dir: &Path) -> Vec<Arc<TrackMetadata>> {
        ...
        tracks.range(playlist_path.clone()..)
    ...
    pub async fn update_index(&self, music_dir: &Path) -> Result<()> {
        ...
        let (tracks_map, playlists) = tokio::task::spawn_blocking(move || {
                    ...
                    if ["flac", "mp3", "m4a", "wav", "ogg", "opus"].contains(&ext.as_str()) {
                        ...
                        if let Ok(meta) = entry.metadata() {
                            ...
                            if let Some(cached) = cache.tracks.get(&path_str) {
                                if cached.file_size == Some(size) && (cached.file_mtime.unwrap_or(0.0) - mtime).abs() < 0.1 {
                                    continue;
                                }
                            }
                            ...
                            if let Ok(probed) = lofty::read_from_path(path) {
                                ...
                                if let Some(tag) = probed.primary_tag() {
                                    if let Some(title) = tag.title() { track.title = title.to_string(); }
                                    if let Some(artist) = tag.artist() { track.artist = artist.to_string(); }
                            ...
                            cache.tracks.insert(path_str, track);
                        }
                    }
                }
            }
            ...
            cache.tracks.retain(|path, _| current_scan_paths.contains(path));
        ...
        }).await.map_err(|e| anyhow::anyhow!("Background task failed: {}", e))?;
